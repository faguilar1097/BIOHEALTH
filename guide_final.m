...
% GUIDE_FINAL MATLAB code for guide_final.fig
%      GUIDE_FINAL, by itself, creates a new GUIDE_FINAL or raises the existing
%      singleton*.
%
%      H = GUIDE_FINAL returns the handle to a new GUIDE_FINAL or the handle to
%      the existing singleton*.
%
%      GUIDE_FINAL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUIDE_FINAL.M with the given input arguments.
%
%      GUIDE_FINAL('Property','Value',...) creates a new GUIDE_FINAL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before guide_final_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guide_final_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help guide_final

% Last Modified by GUIDE v2.5 05-May-2022 23:21:25
...
function guide_final_OpeningFcn(hObject, eventdata, handles, varargin)
global  nombre edad HR textura fecha sexo 
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guide_final (see VARARGIN)
% Choose default command line output for guide_final
...
% UIWAIT makes guide_final wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = guide_final_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global nombre edad HR textura sexo
...
%         close all;
            ...
            title(handles.rojo,'RGB signals');
...
lim = length(Y(1,:));
 x_labelbss='Samples';
 y_labelbss='Amplitude';

plot(handles.canal1,t,Yn(1,:));
title(handles.canal1,'Sources found with BSS');
...
title(handles.picos,'Detected pulses');
...
imshow(rostro);
title(handles.deteccion_rostro,'Automatic face detection');

axes(handles.pomulos);
imshow(pomulos_t);
title(handles.pomulos,'Cheek trimming');

axes(handles.mascara);
imshow(mask);
title(handles.mascara,{['Laws Mask #6']...
...
aget=str2num(get(handles.edad,'String'));
fage =msgbox(["Edad: " num2str(aget)]);
ftxture=msgbox(["Textura: " num2str(skintexture)]);
pause(2);
diagnosticoSkin = TextureDiagnostic(aget,skintexture);
